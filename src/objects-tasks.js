...
class CssSelector {
  ...
  checkOrder(type) {
    const current = this.correctOrder.indexOf(type);

    if (current !== -1) {
      const forbiddenTypes = this.correctOrder.slice(current + 1);

      if (
        forbiddenTypes.some((forbiddenType) =>
          this.usedTypes.includes(forbiddenType)
        )
      ) {
        throw new Error(
